...
disp('Modified by C.C. according to Turone Mouse Brain Template atlas, if encounter problems, please Emil 1510145487@qq.com')
% system settings
try
    spmdir = spm('dir');
    %spm('defaults', 'fmri');
...
xjviewpath = fileparts(which('xjview_TMBT'));
...
f = figure('unit','normalized','position',figurePosition,'Color',figureBKcolor,'defaultuicontrolBackgroundColor', figureBKcolor,...
        'Name','xjView_TMBT', 'NumberTitle','off','resize','on','CloseRequestFcn', {@CallBack_quit, warnstate(1).state}, 'visible','off', 'DoubleBuffer','on');
...
handles.sectionViewTargetFile = getSectionViewTargetFile(spmdir, 'Mouse');
...
xjview_TMBT(cor2mni(coord, handles.M{1}), z);
...
if strcmp(currentsel, 'Mouse')
    xjviewdir = fileparts(which('xjview_TMBT.m'));
    sectionViewTargetFile = fullfile(xjviewdir, 'brain.nii');      
end
return
...
% T = [1.50000000000000,0,0,-96.8000030517578;
%     0,1.50000000000000,0,-179.600006103516;
%     0,0,1.50000000000000,-67.6500015258789;
%     0,0,0,1];
T=[0.6 0 0 -48;
    0 0.6 0 -72;
    0 0 0.6 -34;
...
for ii=1:N
    for jj=1:length(DB)
        ...
        if graylevel == 0
            ...
            tmp = ''; 
%              disp(['jj is',num2str(jj),' gl is ',num2str(graylevel)]);
...
h = spm_render(dat,brt,fullfile(fileparts(which('xjview_TMBT')), 'xjview_render.mat'));
